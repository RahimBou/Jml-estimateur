...
function consolidate(rows,cfg){const m=new Map();for(const r of rows)if(norm(r.nature)==='vente'&&cfg.match(r)&&r.id&&r.price&&r.date){const k=[r.id,r.type_local,r.streetNumber,norm(r.streetName),r.postal,r.parcel].join('|');if(!m.has(k))m.set(k,[]);m.get(k).push(r)}const out=[];for(const a of m.values()){const r=a[0],area=a.reduce((s,x)=>s+(cfg.area==='land'?x.landArea:x.livingArea),0);if(area>0&&r.lat&&r.lon){const rooms=a.reduce((s,x)=>s+(x.rooms||0),0);out.push({...r,area,rooms,units:a.length,sqmPrice:r.price/area})}}return out}
