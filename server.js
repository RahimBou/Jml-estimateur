...
function analyze(rows,input,geo){
  ...
  return{
    ...
    data:{source:'DVF+ géolocalisées — données ouvertes',millime:YEARS.join(', '),engineVersion:'V1.7.0-concurrence-pdf',localCenter:model.localCenterUsed}
  };
}
...
async function searchCompetitionListings(input){
  ...
  const listings=searchListings.map(x=>similarityForCompetition(x,input)).filter(Boolean).slice(0,10);
