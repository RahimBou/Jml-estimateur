...
function buildConfidence(items, filtered, radiusKm, local){
  ...
  const rec=count?filtered.reduce((s,x)=>s+(ageMonths(x.date)||24),0)/count:null;
...
async function estimate(payload){
  ...
  const cfg=TYPE_CONFIG[type];
  const rows=typeRows(all,type).map(x=>({...x,metric:metricValue(x,type)})).filter(x=>x.price>0 && Number.isFinite(x.metric) && x.metric>0 && (cfg.built ? x.area>0 && x.metric<=50000 : true));
