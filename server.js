...
const AGENCY_HOSTS=[
  ...
  'charleville-arthurimmo.com','www.charleville-arthurimmo.com','heros-immobilier.com','www.heros-immobilier.com','alcimmo08.com','www.alcimmo08.com','bressy-immobilier.com','www.bressy-immobilier.com','ardenimmobilier.com','www.ardenimmobilier.com','mathieuimmobilier.fr','www.mathieuimmobilier.fr'
];
...
function extractCompetitionListing(html,url){
  ...
  // Un lotSize/landSize JSON-LD peut être mal renseigné par certains catalogues.
  // On conserve le terrain JSON-LD uniquement s'il est cohérent avec le texte visible.
  const visibleText=String(html).replace(/<[^>]+>/g,' ').replace(/\s+/g,' ');
  const explicitLandValues=[...visibleText.matchAll(/(?:terrain|parcelle|jardin|parc)[^0-9]{0,45}([0-9]{2,6}(?:[.,]\d+)?)\s*m\s*(?:²|2)\b/gi)]
    .map(m=>parseLooseNumber(m[1])).filter(x=>x>=20&&x<=100000);
  const explicitLandAfter=[...visibleText.matchAll(/([0-9]{2,6}(?:[.,]\d+)?)\s*m\s*(?:²|2)\s*(?:de\s*)?(?:terrain|parcelle)\b/gi)]
    .map(m=>parseLooseNumber(m[1])).filter(x=>x>=20&&x<=100000);
  const explicitLandEnsemble=[...visibleText.matchAll(/(?:l['’]ensemble|ensemble)\s+sur\s+([0-9]{2,6}(?:[.,]\d+)?)\s*m\s*(?:²|2)\b/gi)]
    .map(m=>parseLooseNumber(m[1])).filter(x=>x>=20&&x<=100000);
  const explicitLand=[...explicitLandValues,...explicitLandAfter,...explicitLandEnsemble];
  if(best && best.landArea && explicitLand.length && !explicitLand.some(x=>Math.abs(x-best.landArea)<0.01))best.landArea=explicitLand[0];
  if(best && best.landArea && !explicitLand.length)best.landArea=0;
  if(!best){
  ...
  // Une égalité exacte surface = terrain est suspecte si aucun libellé terrain
  // n'est présent : mieux vaut afficher terrain non renseigné que propager une fausse donnée.
  if(best.landArea && Math.abs(best.landArea-best.area)<0.01 && !explicitLand.length)best.landArea=0;
  return {
...
const DIRECT_MARKET_CATALOGUES=[{source:'Héros Immobilier',sourceType:'agence',urls:['https://www.heros-immobilier.com/agences/heros-immobilier-agence-charleville-mezieres/1']},{source:'ALC Immobilier',sourceType:'agence',urls:['https://www.alcimmo08.com/vente/1']},{source:'Dany Bressy Immobilier',sourceType:'agence',urls:['https://www.bressy-immobilier.com/']},{source:'Arden Immobilier',sourceType:'agence',urls:['https://www.ardenimmobilier.com/']},{source:'Mathieu Immobilier',sourceType:'agence',urls:['https://www.mathieuimmobilier.fr/']},{source:'Notaires — immobilier.notaires.fr',sourceType:'notaire',urls:['https://www.immobilier.notaires.fr/fr/annonces-immobilieres-notaires?departement=08']},
...
function listingFromCatalogueText(item,source){
  ...
  // V1.9.17 : certains sites placent le mot "terrain" après la surface
  // (ex. "225 m² terrain"), alors que d'autres écrivent "terrain de 225 m²"
  // ou "parc de 6400 m²". On classe donc chaque surface selon son contexte
  // des deux côtés avant de choisir la surface habitable.
  const contextualAreas=areaMatches.map(m=>{
  ...
  const contextualLiving=contextualAreas.filter(x=>!x.landHint&&x.value<=10000).map(x=>x.value);
  const areas=explicitLiving.length?explicitLiving:(contextualLiving.length?contextualLiving:livingAreas);
  // Les pièces doivent rester distinctes des chambres :
  // "6 pièces / 5 chambres" = 6 pièces, et non 5.
  // On ne déduit plus artificiellement le nombre de pièces à partir du nombre de chambres.
  const roomMatches=[...text.matchAll(/([1-9][0-9]?)\s*(?:pièces?|p\.)/gi)]
    .map(m=>Number(m[1])).filter(x=>x>0&&x<30);
  // Formats immobiliers courants : T2/T3/T4 ou F2/F3/F4.
  // Ils désignent le nombre de pièces et ne doivent pas être confondus avec les chambres.
  const tfMatches=[...text.matchAll(/\b[TF]\s*([1-9][0-9]?)\b/gi)]
    .map(m=>Number(m[1])).filter(x=>x>0&&x<30);
  const bedroomMatches=[...text.matchAll(/([1-9][0-9]?)\s*chambres?/gi)]
    .map(m=>Number(m[1])).filter(x=>x>0&&x<30);
  const rooms=roomMatches.length?roomMatches:(tfMatches.length?tfMatches:[]);
  const landPatterns=[
    /(?:terrain|parcelle|jardin|parc)[^0-9]{0,45}([0-9]{2,6}(?:[.,]\d+)?)\s*m\s*(?:²|2)\b/gi,
    /([0-9]{2,6}(?:[.,]\d+)?)\s*m\s*(?:²|2)\s*(?:terrain|parcelle)\b/gi
  ];
  const land=[];
  for(const reLand of landPatterns){
    for(const m of text.matchAll(reLand)){
      const value=parseLooseNumber(m[1]);
      if(value>=20&&value<=100000)land.push(value);
    }
  }
...
async function enrichCatalogueListing(item){
  ...
  try{
    const html=await fetchPublicPage(item.url);
    const detail=extractCompetitionListing(html,item.url);
    if(!detail)return item;
    return {
      ...item,
      // La fiche détaillée est prioritaire pour les informations absentes du catalogue.
      price:detail.price||item.price,
      area:detail.area||item.area,
      rooms:detail.rooms||item.rooms,
      // Une fiche détaillée peut fournir la vraie surface habitable et le terrain.
      // On ne remplace jamais une donnée fiable par zéro.
      landArea:detail.landArea||item.landArea,
      locality:detail.locality||item.locality,
      sqmPrice:(detail.area||item.area)?Math.round((detail.price||item.price)/(detail.area||item.area)):item.sqmPrice,
      detailPage:true
    };
  }catch{
    return item;
...
async function searchDirectAgencyCatalogues(input,city,typeLabel){
  ...
  const base=[...unique.values()].slice(0,25);
  // Après le catalogue, on lit les fiches individuelles pour récupérer les champs
  // que la carte de catalogue ne contient pas (notamment pièces et terrain).
  const enriched=[];
  for(let i=0;i<base.length;i+=8){
    const batch=await Promise.all(base.slice(i,i+8).map(enrichCatalogueListing));
    enriched.push(...batch);
  }
  return enriched;
}
...
async function searchCompetitionListings(input){
  ...
  const agencies=[
    ...
    'charleville-arthurimmo.com','heros-immobilier.com','alcimmo08.com','bressy-immobilier.com','ardenimmobilier.com','mathieuimmobilier.fr'
  ];
  const ardennesTowns=[
  ...
  for(const raw of merged){
    const x={...raw};
    // Sécurité anti-fausse surface : certains sites fournissent dans JSON-LD
    // un lotSize identique à la surface habitable alors que l'annonce ne mentionne
    // aucun terrain. Pour la liste du marché, on n'affiche pas cette valeur sans
    // indication terrain dans le titre/extrait de l'annonce.
    const listingText=norm(String(x.title||'')+' '+String(x.snippet||''));
    const hasExplicitLand=/(?:terrain|parcelle|jardin|parc)\\s*(?:de|d['’]environ|d['’])?\\s*[:=]?\\s*\\d|\\d[0-9., ]*\\s*m(?:²|2)\\s*(?:de\\s*)?(?:terrain|parcelle)\\b/.test(listingText);
    if(x.fromCatalogue && x.area>0 && x.landArea>0 && Math.abs(Number(x.area)-Number(x.landArea))<0.01 && !hasExplicitLand){
      x.landArea=0;
    }
    const key=x.url||[x.source,x.price,x.area,x.rooms,norm(x.title)].join('|');
    if(!finalMap.has(key))finalMap.set(key,x);
  }
